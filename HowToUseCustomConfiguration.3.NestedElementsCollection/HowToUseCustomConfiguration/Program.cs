...
namespace HowToUseCustomConfiguration
{
    class Program
    {
        ...
        private static void DumpCustomConfigSection()
        {
            ...
            for (int i = 0; i < cfg.NestedElements.Count; i++)
            {
                Console.WriteLine("-- nested {0} --", i);
                Console.WriteLine("Name         : {0}", cfg.NestedElements[i].Name);
                Console.WriteLine("TextValue    : {0}", cfg.NestedElements[i].TextValue);
                Console.WriteLine("NumericValue : {0}", cfg.NestedElements[i].NumericValue);
            }
        }
    }
}
